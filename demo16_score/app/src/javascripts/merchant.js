...
module.exports = {
  ...
  newMerchant: function (ScoreInstance, account) {
    ...
    ScoreInstance.newMerchant(address, password, { from: account, gas: 1000000 }).then(function (r,e) {
      // ScoreInstance.NewMerchant(function (error, event) {
      if (!e) {
        console.log(r.logs[0].args)
        if (r.logs[0].args.isSuccess === true) {
  ...
  transferScoreToAnotherFromMerchant: function (currentAccount, ScoreInstance, account) {
    ...
    ScoreInstance.transferScoreToAnother(1, currentAccount, receivedAddr, amount, { from: account }).then(function(r,error) {
      if (!error) {
  ...
  addGood: function (currentAccount, ScoreInstance, account) {
    ...
    ScoreInstance.addGood(currentAccount, goodId, goodPrice, { from: account, gas: 2000000 }).then(function (r,error) {
  ...
  settleScoreWithBank: function (currentAccount, ScoreInstance, account) {
    ...
    ScoreInstance.settleScoreWithBank(currentAccount, settleAmount, { from: account }).then(function (r,error) {
